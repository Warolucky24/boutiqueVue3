...
<template>
  <div class="p-8 d_flex flex_column">
    <section class="mb_20">
      <h3 class="mb_10 font-bold">Rechercher :</h3>
      <input
        type="text"
        @input="
          emit('updateFilter', {
    ...
    <section class="mb_20">
      <h3 class="font-bold my-1">Trier par prix :</h3>
      <div
        class="mb_5"
        v-for="priceRange in ([[0,10000],[600,1000],[1000,1300],[1300,1500]] as [number, number][])"
      >
        <input
        ...
        <label :for="priceRange[0] + ''">
          {{
            priceRange[0] === 0
              ? "Tous les prix"
              : priceRange[0] + " à " + priceRange[1]
          }}
        </label>
      </div>
    </section>
    <section class="mb_20">
      <h3 class="mb_10 font-bold">Trier par catégories :</h3>
      <p
        class="category"
    ...
    <section class="d_flex flex_column flex_fill">
      <button class="btn btn_danger" @click="emit('updateFilter', {})">
        Reset filtre
      </button>
    </section>
  </div>
